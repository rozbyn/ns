...
<body>
	...
	<script>
		...
		window.onload = addEventFiles(files);
			...
			for(var i=0;i<len;i++){
				htmlCollect[i].addEventListener("click", function () {
					...
					ajaxPost('explorer.php', params, this, function (data, s) {
						...
						var files = p.getElementsByClassName('expl_file');
		...
		function addEventFiles (htmlCollect) {
			var len = htmlCollect.length;
			for(var i=0;i<len;i++){
				htmlCollect[i].addEventListener("click", function (e) {
					redirectTo = '';
					if (e.target.nodeName === 'DIV') {
						redirectTo = e.target.firstElementChild.getAttribute('href');
					} else {
